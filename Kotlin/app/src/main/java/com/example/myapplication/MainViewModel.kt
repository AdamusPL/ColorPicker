...
class MainViewModel : ViewModel() {
    ...
    val cmykFieldValues: SnapshotStateList<String> = mutableStateListOf("0%", "0%", "0%", "0%")
    val hexFieldValues: SnapshotStateList<String> = mutableStateListOf("#000000")

    fun updateColor(hasChanged : String) { when (hasChanged) {
            "RGB" -> {
            ...
            "HSV" -> {
                val h = hsvFieldValues[0].removeSuffix("°").toDoubleOrNull() ?: 0.0
                val s = hsvFieldValues[1].removeSuffix("%").toDoubleOrNull() ?: 0.0
                val v = hsvFieldValues[2].removeSuffix("%").toDoubleOrNull() ?: 0.0
            ...
            "CMYK" -> {
                val c = cmykFieldValues[0].removeSuffix("%").toIntOrNull() ?: 0
                val m = cmykFieldValues[1].removeSuffix("%").toIntOrNull() ?: 0
                val y = cmykFieldValues[2].removeSuffix("%").toIntOrNull() ?: 0
                val k = cmykFieldValues[3].removeSuffix("%").toIntOrNull() ?: 0
            ...
            "HEX" -> {
                val hex = hexFieldValues[0].removePrefix("#")
